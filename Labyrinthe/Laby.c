...
void init(int t,Case** tabLaby, int** typeCase){
    ...
    tabLaby[entrer][0].up = 0;
    tabLaby[entrer][0].right = 1;
    tabLaby[entrer][0].down = 0;
    tabLaby[entrer][0].left = 1;
 
    tabLaby[sortie1][(t-1)].up = 0;
    tabLaby[sortie1][(t-1)].right = 1;
    tabLaby[sortie1][(t-1)].down = 0;
    tabLaby[sortie1][(t-1)].left = 1;
 
    tabLaby[sortie2][(t-1)].up = 0;
    tabLaby[sortie2][(t-1)].right = 1;
    tabLaby[sortie2][(t-1)].down = 0;
    tabLaby[sortie2][(t-1)].left = 1;
}
 
int checkDoor(int t, Case ** tabLaby){
    //tabPrinter(t, tabLaby);
    int var = 0;
    for (int i = 1; i < t-1; i++){
        for(int j = 1; j < t-1; j++){
            if (tabLaby[i][j].check != 0) var = 1; //return 0;
        }
    }
    if (var == 1) return 0;
    return 1;
...
int main() {
    srand(time(NULL));
    Case ** tabLaby = (Case **)malloc(tailleLab * sizeof(Case *)+25);
    for (int i = 0; i < tailleLab; ++i) {
        tabLaby[i] = (Case *)malloc(tailleLab * sizeof(Case)+25);
    }
 
    int ** typeCase = (int **)malloc(16 * sizeof(int *)+25);
    for (int i = 0; i < 16; ++i) {
        typeCase[i] = (int *)malloc(4 * sizeof(int)+25);
    }
    ...
    for (int j = 0; j < 16; j++) {
            printf("%d : %d, %d, %d, %d\n", j, typeCase[j][0], typeCase[j][1], typeCase[j][2], typeCase[j][3]);
    }
